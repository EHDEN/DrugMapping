...
public class DBConnector {
	...
	public static Connection connect(String server, String domain, String user, String password, DbType dbType) {
		...
		else if (dbType.equals(DbType.MSAZURE))
			return DBConnector.connectToMSAZURE(server, domain, user, password);
	...
	public static Connection connectToMSAZURE(String server, String domain, String user, String password) {
		if (user == null || user.length() == 0) { // Use Windows integrated security
			try {
				Class.forName("com.microsoft.sqlserver.jdbc.SQLServerDriver");
			} catch (ClassNotFoundException e1) {
			...
			String url = "jdbc:jtds:sqlserver://" + server + ":1433;" + ";encrypt=true;trustServerCertificate=false;hostNameInCertificate=*.database.windows.net;loginTimeout=30;";
